            ...
            <div className="body_center" style={{ pointerEvents: connectionEstablished ? "" : "none" }}>
                {connectionEstablished ?
                    <div style={{ margin: '25px' }}>
                        {gameStart ? <div>{zeroTurn ? '0 turn now' : 'X turn now'}{(peerRef.current.initiator && zeroTurn) || (!peerRef.current.initiator && !zeroTurn) ? '(your turn)' : '(his turn)'}</div> : <div>{!initialNKRequest ? 'Waiting for acceptance':'Accept To Start Game'}</div>}
                    </div> :
